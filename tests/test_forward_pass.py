...
class TestWorldModelSpaceTime:
    """Tests for space/time factorized attention (Dreamer 4 pattern)."""
    ...
    def test_forward_deterministic(self, device, seed):
        ...
        tokens, actions = self._make_inputs(device)
        with torch.no_grad():
            logits1, _ = model(tokens, actions)
            logits2, _ = model(tokens, actions)
        assert torch.allclose(logits1, logits2, atol=1e-5), "Forward not deterministic"
    ...
    def test_backward_flows_to_all_blocks(self, device, seed):
        from deepdash.world_model import WorldModel
        model = WorldModel(
            vocab_size=1000, embed_dim=512, n_heads=8, n_layers=8,
            context_frames=4, tokens_per_frame=64, adaln=True,
        ...
        tokens = torch.randint(0, 1000, (2, 5, 65), device=device)
        tokens[:, :, -1] = 1000
        actions = torch.randint(0, 2, (2, 4), device=device)
        logits, cpc = model(tokens, actions)
        loss = logits.mean() + cpc
        loss.backward()
        for i, block in enumerate(model.blocks):
            g = block.qkv.weight.grad
            assert g is not None, f"block {i} qkv.weight has no grad"
            assert g.abs().sum().item() > 0, f"block {i} qkv grad is all-zero"

    def test_requires_adaln(self, device, seed):
        from deepdash.world_model import WorldModel
        with pytest.raises(AssertionError, match="requires adaln=True"):
            WorldModel(
                vocab_size=1000, embed_dim=512, n_heads=8, n_layers=8,
                context_frames=4, tokens_per_frame=64, adaln=False,
                attention_pattern="space_time_sst_t",
            )

    def test_requires_layers_div_by_4(self, device, seed):
        from deepdash.world_model import WorldModel
        with pytest.raises(AssertionError, match="divisible by 4"):
            WorldModel(
                vocab_size=1000, embed_dim=512, n_heads=8, n_layers=6,
                context_frames=4, tokens_per_frame=64, adaln=True,
                attention_pattern="space_time_sst_t",
            )

    def test_rejects_unknown_pattern(self, device, seed):
        from deepdash.world_model import WorldModel
        with pytest.raises(ValueError, match="Unknown attention_pattern"):
            WorldModel(
                vocab_size=1000, embed_dim=512, n_heads=8, n_layers=8,
                context_frames=4, tokens_per_frame=64, adaln=True,
